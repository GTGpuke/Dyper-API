...
async def transcribe_file(audio_path: str) -> str | None:
    """Transcrit un fichier audio via Whisper (Groq). Retourne None sur tout échec."""
    if not is_available():
        return None
    try:
        client = _get_client()
...
async def analyze_audio(video_path: str) -> tuple[str | None, MusicInfo | None]:
    """Analyse complète de la piste audio : transcription + reconnaissance musicale.
    ...
    try:
        transcript, music = await asyncio.gather(
